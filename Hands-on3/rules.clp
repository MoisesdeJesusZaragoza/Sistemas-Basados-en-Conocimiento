				        	...
				        	(assert (print-vals Migraine headache nausea))
					  	    ...
					  	    then
				        		(assert (print-vals Hypertension fatigue "blurred vision"))
                                    ...
                                    (if(eq ?illness HEPATITIS)
                                then
                                   (assert (print-vals pain, fatigue, cloudy urine))
                                   else
                                   (if (eq ?illness TUBERCULOSIS)
                                   then
                                      (assert (print-vals Cough, chest pain, fatigue, fever, chills))
                                    else
                                      (if (eq ?illness SMALLPOX)
                                    then
                                      (assert(print-vals high fever, headache and body pain, and sometimes vomiting))
                                      else
                                      (if (eq ?illness SYPHILIS)
                                      then
                                           (assert(print-vals hills, fever, headache, nausea, rash))
                                           else
